...
- **Invoke the `tailwind-4-docs` skill** before writing any Tailwind CSS code, every session, no exceptions.
- **Use the `mcp__astro-docs__search_astro_docs` tool** before writing any Astro code — never assume APIs or syntax, always look it up first.
...
- Start the dev server: `yarn dev` (serves the project root at `http://localhost:4321`)
